...
btinfo <- btinfo[!duplicated(rarefyID), .(rarefyID, rarefyID_x, rarefyID_y, Biome, taxa_mod, S, N, REALM, STUDY_ID)]
bt <- merge(bt, btinfo, by = 'rarefyID', all.x = TRUE)

# richness
rich <- fread('output/richness_by_rarefyID.csv.gz') # number of species
```

## Examine sample frequency and duration
```{r}
durations <- bt[, .(year1 = min(year1), year2 = max(year2), numsamps = length(unique(year2))+1), by = rarefyID][, .(duration = year2 - year1, numsamps), by = rarefyID]

# how many studies with >= X years and annual sampling?
durations[duration >= 3 & numsamps == duration+1, .N]
durations[duration >= 5 & numsamps == duration+1, .N]
durations[duration >= 10 & numsamps == duration+1, .N]
durations[duration >= 20 & numsamps == duration+1, .N]
```

## Examine number of species and individuals
```{r}
hist(log10(btinfo$N))
...
```{r ave change vs. duration, message = FALSE}
