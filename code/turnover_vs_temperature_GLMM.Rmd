...
AICtab(modRFgauss, modRFbeta, modRFrID, modRF2lev, modRFnestedRE, modRFdisp, modRFdisp2lev, modRFdisp2levOnlyint,
       modRFdurslope2lev, modRFdurslope2levdisprealm)
```
Chooses beta errors, random slopes (duration.sc) & intercepts for STUDY_ID and rarefyID, and variance scaled to nspp + realm.
We haven't dealt with potential testing on the boundary issues here yet.

### DHARMa simulate residuals
```{r}
if(!file.exists('temp/res_RFdurslope2levdisprealm.rds')) {
  ...
  res_RFdurslope2levdisprealm <- readRDS('temp/res_RFdurslope2levdisprealm.rds')
}
```
...
#### Simulate residuals
```{r}
if(!file.exists('temp/res_modTDrealmJtu.rds')) {
  ...
  res_modTDrealmJtu <- readRDS('temp/res_modTDrealmJtu.rds')
}

if(!file.exists('temp/res_modTrealm1yrJtu.rds')) {
  ...
  res_modTrealm1yrJtu <- readRDS('temp/res_modTrealm1yrJtu.rds')
}


if(!file.exists('temp/res_modTrealm10yrJtu.rds')) {
  i <- trends[, complete.cases(Jtu.sc, tempchange, REALM, nspp.sc) &
  ...
  res_modTrealm10yrJtu <- readRDS('temp/res_modTrealm10yrJtu.rds')
}
```
...
```
nohup code/turnover_vs_temperature_GLMM_pred.R temp/modFullMaEnMiNPHuJtu.rds temp/newdata_main.csv > logs/turnover_vs_temperature_GLMMmodFullMaEnMiNPHuJtu_pred.Rout &
```
...
Now go run on the command line
```
nohup code/turnover_vs_temperature_GLMM_pred.R temp/modFullMaEnMiNPHuJtu.rds temp/newdata_interactions.csv > logs/turnover_vs_temperature_GLMMmodFullMaEnMiNPHuJtu_pred_interactions.Rout &
```

##### Load and plot
```{r}
newdat <- readRDS('temp/modFullMaEnMiNPHuJtu_preds_interactions.rds')
newdat[, REALM := factor(REALM, levels = c('Freshwater', 'Terrestrial', 'Marine'))] # set factor order
newdat[, REALM2 := factor(REALM2, levels = c('TerrFresh', 'Marine'))] # set factor order
...
# prep the plots
ints <- unique(newdat[ ,.(var, REALM2, log, discrete)])
intplots <- vector('list', nrow(ints))
for(j in 1:length(intplots)){
  subs <- newdat$var == ints$var[j] & newdat$tempchange > 0 # select warming side
  xvar <- 'tempchange_abs'
  title <- ints$var[j]
  if(ints$var[j] %in% c('human_bowler')){
    subs <- newdat$var == ints$var[j] & newdat$tempchange > 0 & newdat$REALM2 == ints$REALM2[j]
