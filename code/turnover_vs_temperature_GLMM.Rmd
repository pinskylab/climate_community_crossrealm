...
if(file.exists('temp/modRFdisp2lev.rds')) modRFdisp2levOnlyint <- readRDS('temp/modRFdisp2levOnlyint.rds')


AICtab(modRFgauss, modRFbeta, modRFrID, modRFnestedRE, modRFdisp2levOnlyint)
...
# write out
write.csv(newdat, 'temp/newdata.csv')
...
```{r}
if(file.exists('temp/modFullMaEnMiNPHuJtu_preds.rds')) {
  newdat <- readRDS('temp/modFullMaEnMiNPHuJtu_preds.rds')
}

ggplot(newdat, aes(tempchange, pred)) +
  geom_line() +
