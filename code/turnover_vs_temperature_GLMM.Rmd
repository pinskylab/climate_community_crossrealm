...
# predict
newdatM$Jtu.sc <- predict(modTsdTTRealmDurscAllJtu, newdata = newdatM, se.fit = FALSE, re.form = NA, allow.new.levels=TRUE)
newdatT$Jtu.sc <- predict(modTsdTTRealmDurscAllJtu, newdata = newdatT, se.fit = FALSE, re.form = NA, allow.new.levels=TRUE)
newdatF$Jtu.sc <- predict(modTsdTTRealmDurscAllJtu, newdata = newdatF, se.fit = FALSE, re.form = NA, allow.new.levels=TRUE)
...
### Load the models
```{r, echo=FALSE}
modnames <- c('modNull', 'modRealm', 'modT', 'moddT', 'modsdT', 'modmass', 'modnpp', 'modseas', 'modmicroclim', 'modhuman', 'modTdT', 'modTRealm', 'modTmass', 'modTnpp', 'modTseas', 'modTmicroclim', 'modThuman', 'moddTRealm', 'moddTtsign', 'moddTmass', 'moddTnpp', 'moddTseas', 'moddTmicroclim', 'moddThuman', 'modsdTRealm', 'modsdTmass', 'modsdTnpp', 'modsdTseas', 'modsdTmicroclim', 'modsdThuman', 'modTdTRealm', 'modTdTtsign', 'modTdTmass', 'modTdTnpp', 'modTdTseas', 'modTdTmicroclim', 'modTdThuman', 'modTdTT', 'modTsdTT', 'modTxRealm', 'modTxmass', 'modTxnpp', 'modTxseas', 'modTxmicroclim', 'modTxhuman', 'moddTxRealm', 'moddTxmass', 'moddTxnpp', 'moddTxseas', 'moddTxmicroclim', 'moddTxhuman', 'modsdTxRealm', 'modsdTxmass', 'modsdTxnpp', 'modsdTxseas', 'modsdTxmicroclim', 'modsdTxhuman')
...
checkconv <- function(x){
  if(is.null(x)){
    return(-1) # if nothing fit
  } else {
    return(x$fit$convergence) # otherwise return convergence code. 0 is converged.
  }
}
...
daics <- data.frame(num=(1:length(aics))[good], daic=aics - min(aics))
daics[daics[,2] < 5 | grepl('Null', rownames(daics)), ] # print null and dAIC<4
```
###### sdT vs. dT
```{r}
daics[grepl('moddT5|modsdT5', rownames(daics)), ] # print signed and abs tempchange
```
...
cbind((1:length(aics))[good], aics - min(aics))
```
