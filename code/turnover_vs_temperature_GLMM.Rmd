...
fixed <- formula(Jtu.sc ~ tempchange_abs.sc*REALM +
...
mods <- vector('list', 0)
mods[[1]] <- glmmTMB(fixed, data = trends[i,], family = gaussian(), control = glmmTMBControl(profile=TRUE))
mods[[2]] <- glmmTMB(fixed, data = trends[i,], family = beta_family(link = 'logit')) # add beta errors
mods[[3]] <- glmmTMB(paste(fixed, '+(1|rarefyID)'), data = trends[i,], family = list(family = 'beta', link = 'logit')) # add random effects
