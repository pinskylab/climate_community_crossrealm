...
## Temperature*temperature slope*REALM*duration
...
if(file.exists(here('temp', 'slopes_TsdTTRealm.rds'))){
  ...
  # calculate slopes and the SE of the slope from the amount the points don't fit a line
  slopes <- newdat[, .(slope = coef(lm(Jtu.sc ~ duration))[2], slope.lm.se = sqrt(diag(vcov(lm(Jtu.sc ~ duration))))[2]), by = .(tempave_metab, tempchange_abs, REALM)]

  # calculate SE on the slopes from the error in the points (but assumes points fit a line)  
  source(here('code', 'util.R'))
  coefs <- newdat[, ellipse.coefs(cbind(duration, Jtu.sc, Jtu.sc.se), alpha = 0.05), by = .(tempave_metab, tempchange_abs, REALM)]
  xy <- coefs[, get.ellipse(a, b, c, d, e, f), by = .(tempave_metab, tempchange_abs, REALM)] # produces some NaNs
  cis <- xy[, .(slope.ellipse.se = diff(range(x))/1.96/2), by = .(tempave_metab, tempchange_abs, REALM)] # the SE for the slope, converted from the 95% CI
  slopes2 <- merge(slopes, cis)
  
  # sum the SE from the two sources
  slopes2[, slope.ellipse.se_no0 := slope.ellipse.se]
  slopes2[is.na(slope.ellipse.se), slope.ellipse.se_no0 := 0] # turn NaNs to zeros
  slopes2[, slope.total.se := sqrt(slope.lm.se^2 + slope.ellipse.se_no0^2)]
  slopes2[, slope.ellipse.se_no0 := NULL]
  
  saveRDS(slopes2, file = here('temp', 'slopes_TsdTTRealm.rds'))
}
```
...
Three approaches: 1) use the CI on the linear regression, or 2) use the SE on the points themselves assuming that the points fit a line, or 3) use the sum of the variances
```{r, fig.height=8}
# first approach:
p1 <- ggplot(slopes[round(tempave_metab,1) %in% c(10.1, 30.2),], aes(tempchange_abs, slope, color = factor(round(tempave_metab)), group = tempave_metab)) +
    geom_line() +
    geom_pointrange(aes(ymin=slope-1.96*slope.lm.se, ymax=slope+1.96*slope.lm.se), alpha = 0.25) + 
...
# SE of the points approach (ellipse)
p2 <- ggplot(slopes2[round(tempave_metab,1) %in% c(10.1, 30.2),], aes(tempchange_abs, slope, color = factor(round(tempave_metab)), group = tempave_metab)) +
    geom_line() +
    geom_pointrange(aes(ymin=slope-1.96*slope.ellipse.se, ymax=slope+1.96*slope.ellipse.se), alpha = 0.25) + 
    facet_grid(cols = vars(REALM))  +
    labs(x = '|dT|') +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
          panel.background = element_blank(), axis.line = element_line(colour = "black"),
...
# sum
p3 <- ggplot(slopes2[round(tempave_metab,1) %in% c(10.1, 30.2),], aes(tempchange_abs, slope, color = factor(round(tempave_metab)), group = tempave_metab)) +
    geom_line() +
    geom_pointrange(aes(ymin=slope-1.96*slope.total.se, ymax=slope+1.96*slope.total.se), alpha = 0.25) + 
    facet_grid(cols = vars(REALM))  +
    labs(x = '|dT|') +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
          panel.background = element_blank(), axis.line = element_line(colour = "black"),
...
## Covariate*Temperature*temperature slope*REALM*duration
```{r, echo = FALSE}
