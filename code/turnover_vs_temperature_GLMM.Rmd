...
AICtab(modRFgauss, modRFbeta, modRFrID, modRFnestedRE, modRFslopeRE, modRFdisp, modRFdisp2lev)
...
##### Simulate residuals
```{r dharma maenminphu}
if(!file.exists('temp/res_MaEnMiNPHu.rds')) {
  # make a vector of 1000 values that meet our completeness criteria
  i <- trends[, complete.cases(Jtu.sc, tempchange_abs.sc, REALM, tempave_metab.sc, duration.sc, mass.sc, endothermfrac.sc,
                               microclim.sc, npp.sc, human_bowler.sc)] # needed because model fit with trends[i,]

  res_MaEnMiNPHu <- simulateResiduals(modFullMaEnMiNPHu, n = 100)
  
  saveRDS(res_MaEnMiNPHu, file = 'temp/res_MaEnMiNPHu.rds')
} else {
  i <- trends[, complete.cases(Jtu.sc, tempchange_abs.sc, REALM, tempave_metab.sc, duration.sc, mass.sc, endothermfrac.sc,
                               microclim.sc, npp.sc, human_bowler.sc)] # needed because model fit with trends[i,]
  res_MaEnMiNPHu <- readRDS('temp/res_MaEnMiNPHu.rds')
}
```
...
plot(res_MaEnMiNPHu)
plotResiduals(res_MaEnMiNPHu, form=trends[i,REALM])
plotResiduals(res_MaEnMiNPHu, trends[i,tempave_metab.sc])
plotResiduals(res_MaEnMiNPHu, trends[i,duration.sc])
plotResiduals(res_MaEnMiNPHu, trends[i,mass.sc])
plotResiduals(res_MaEnMiNPHu, trends[i,endothermfrac.sc])
plotResiduals(res_MaEnMiNPHu, trends[i,microclim.sc])
plotResiduals(res_MaEnMiNPHu, trends[i,npp.sc])
plotResiduals(res_MaEnMiNPHu, trends[i,human_bowler.sc])
```