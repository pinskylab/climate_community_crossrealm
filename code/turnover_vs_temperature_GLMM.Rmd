...
newdatM <- data.table(expand.grid(tempave_metab.sc = seq(-1.6, 1.5, length.out = 100), tempchange_abs.sc = seq(-0.8, 20, length.out = 100), duration = seq(1, 10, length.out=10)))
...
newdatT <- data.table(expand.grid(tempave_metab.sc = seq(-1.6, 1.5, length.out = 100), tempchange_abs.sc = seq(-0.8, 20, length.out = 100), duration = seq(1, 10, length.out=10)))
...
newdatF <- data.table(expand.grid(tempave_metab.sc = seq(-1.6, 1.5, length.out = 100), tempchange_abs.sc = seq(-0.8, 20, length.out = 100), duration = seq(1, 10, length.out=10)))
...
# predict
newdatM$Jtu.sc <- predict(modTsdTTRealmAllJtu, newdata = newdatM, se.fit = FALSE, re.form = NA, allow.new.levels=TRUE, type = 'response')
newdatT$Jtu.sc <- predict(modTsdTTRealmAllJtu, newdata = newdatT, se.fit = FALSE, re.form = NA, allow.new.levels=TRUE, type = 'response')
newdatF$Jtu.sc <- predict(modTsdTTRealmAllJtu, newdata = newdatF, se.fit = FALSE, re.form = NA, allow.new.levels=TRUE, type = 'response')

# calculate slopes
slopesM <- newdatM[, .(slope = coef(lm(Jtu.sc ~ duration))[2]), by = .(tempave_metab, tempchange_abs)]
slopesT <- newdatT[, .(slope = coef(lm(Jtu.sc ~ duration))[2]), by = .(tempave_metab, tempchange_abs)]
slopesF <- newdatF[, .(slope = coef(lm(Jtu.sc ~ duration))[2]), by = .(tempave_metab, tempchange_abs)]
...
### Tsign summary
```{r}
summary(modTsdTTRealmtsignAllJtu)

```
### Plot temperature sign effect
```{r, echo = FALSE}
# set up prediction frame
...
### AIC
```{r}
aics <- AIC(modTsdTTRealmAllJtu, modTsdTTRealmtsignAllJtu, modTsdTTRealmnppAllJtu, modTsdTTRealmseasAllJtu, modTsdTTRealmmicroclimAllJtu)
aics$dAIC <- aics$AIC - min(aics$AIC)
aics
```
...
### Plot the interaction for high and low microclimates
```{r, echo = FALSE}
# set up prediction frame
...
newdat2$STUDY_ID <- 1
newdat2$rarefyID <- 1
newdat2[, tempave_metab.sc := (tempave_metab - scalingall[var == 'tempave_metab.sc', center]) / scalingall[var == 'tempave_metab.sc', scale]]
newdat2[, tempchange_abs.sc := (tempchange_abs - scalingall[var == 'tempchange_abs.sc', center]) / scalingall[var == 'tempchange_abs.sc', scale]]
newdat2[, microclim := microclim.sc * scalingall[var == 'microclim.sc', scale] + scalingall[var == 'microclim.sc', center]]
newdat2[, npp := npp.sc * scalingall[var == 'npp.sc', scale] + scalingall[var == 'npp.sc', center]]
newdat2[, seas := seas.sc * scalingall[var == 'seas.sc', scale] + scalingall[var == 'seas.sc', center]]
...
slopes2 <- newdat2[, .(slope_microclim = coef(lm(Jtu.sc.microclim ~ duration))[2],
                   ...
                   by = .(tempave_metab, tempchange_abs, microclim, npp, seas, REALM)]

# normal slopes to 0
zeros <- slopes2[tempchange_abs == 0, .(tempave_metab, microclim, npp, seas, REALM, 
                                        slope_microclim0 = slope_microclim, slope_npp0 = slope_npp, slope_seas0 = slope_seas)]
slopes2 <- merge(slopes2, zeros, all.x = TRUE)
slopes2[, ':='(slope_microclim.n = slope_microclim - slope_microclim0, slope_npp.n = slope_npp - slope_npp0, slope_seas.n = slope_seas - slope_seas0)]
