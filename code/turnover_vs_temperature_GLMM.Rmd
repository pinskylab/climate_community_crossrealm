...
p3 <- ggplot(slope10, aes(x=tempchange_abs, y=Hornslope, ymin=HornslopedTu, ymax=HornslopedTl)) +
  geom_line() + 
  geom_ribbon(alpha=0.2)
  
p4 <- ggplot(slope10, aes(x=tempave_metab, y=Hornslope, ymin=HornslopeTu, ymax=HornslopeTl)) +
  geom_line() +
  geom_ribbon(alpha=0.2)

p5 <- ggplot(slope20, aes(x=tempchange_abs, y=Hornslope, ymin=HornslopedTu, ymax=HornslopedTl)) +
  geom_line() + 
  geom_ribbon(alpha=0.2)
  
p6 <- ggplot(slope20, aes(x=tempave_metab, y=Hornslope, ymin=HornslopeTu, ymax=HornslopeTl)) +
  geom_line() +
  geom_ribbon(alpha=0.2)
...
### Load the models
```{r, echo=FALSE}
modnames <- c('modNull', 'modRealm', 'modT', 'moddT', 'modmass', 'modnpp', 'modseas', 'modmicroclim', 'modhuman', 'modTdT', 'modTmicroclim', 'moddTmicroclim', 'modTdTmicroclim', 'modTdTT', 'modTdTTRealm', 'modTdTTtsign', 'modTdTTmass', 'modTdTTnpp', 'modTdTTseas', 'modTdTTmicroclim', 'modTdTThuman')
...
print(nobs)
modes <- sapply(X = mods5Jtu, FUN = mode)
if(!all(unlist(nobs[modes == 'list']) == nobs[modes == 'list'][1])) print('NOT ALL THE SAME! AIC is meaningless')
aics <- sapply(X = mods5Jtu[modes == 'list'], FUN = AIC)
t(t(aics - min(aics)))
```
...
if(!all(unlist(nobs[modes == 'list']) == nobs[modes == 'list'][1])) print('NOT ALL THE SAME! AIC is meaningless')
modes <- sapply(X = mods10Jtu, FUN = mode)
aics <- sapply(X = mods10Jtu[modes == 'list'], FUN = AIC)
t(t(aics - min(aics)))
```
...
if(!all(unlist(nobs[modes == 'list']) == nobs[modes == 'list'][1])) print('NOT ALL THE SAME! AIC is meaningless')
aics <- sapply(X = mods20Jtu[modes == 'list'], FUN = AIC)
t(t(aics - min(aics)))
...
print(nobs)
if(!all(nobs == nobs[1])) print('NOT ALL THE SAME! AIC is meaningless')
modes <- sapply(X = mods5Horn, FUN = mode)
aics <- sapply(X = mods5Horn[modes == 'list'], FUN = AIC)
t(t(aics - min(aics)))
```
...
aics <- sapply(X = mods10Horn[modes == 'list'], FUN = AIC)
t(t(aics - min(aics)))
```
...
aics <- sapply(X = mods20Horn[modes == 'list'], FUN = AIC)
t(t(aics - min(aics)))
```
