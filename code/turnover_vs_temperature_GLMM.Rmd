...
modrawTsdTTRealmtsignmicroclimhumanAllJtu <- readRDS(here('temp', 'modrawTsdTTRealmtsignmicroclimhumanAllJtu.rds')) # seas
...
##### seasonality model
Predictions were only made for a few tempaves, so the plot shows only strips.  
However, matches well to model w/out seasonality
```{r, echo = FALSE}
# plot
p1 <- ggplot(slopes_rawTsdTTRealmtsignCovariate[seas == max(seas), ], aes(tempchange, tempave, z = slope_seas)) +
