...
CIs are from a SEM fit to the trends.
```{r, fig.height=8}
ggplot(slopesrawTdTT[round(tempave,1) %in% c(10.0, 21.6),], aes(tempchange, slope, color = factor(round(tempave)), group = tempave)) +
    geom_line() +
    geom_pointrange(aes(ymin=slope-1.96*slope.se, ymax=slope+1.96*slope.se), alpha = 0.25) + 
    facet_grid(cols = vars(REALM))  +
    labs(x = 'dT [degC/yr]') +
...
CIs are from a SEM fit to the trends.
```{r, fig.height=8}
ggplot(slopesrawTsdTT[round(tempave,1) %in% c(10.0, 21.6),], aes(tempchange_abs, slope, color = factor(round(tempave)), group = tempave)) +
    geom_line() +
    geom_pointrange(aes(ymin=slope-1.96*slope.se, ymax=slope+1.96*slope.se), alpha = 0.25) + 
    facet_grid(cols = vars(REALM))  +
    labs(x = '|dT|') +
