...
## Summaries
```{r}
if(!exists('sum.init.null')) sum.init.null <- summary(modInitAllJtu)
if(!exists('sum.init.Tchange')) sum.init.Tchange <- summary(modsdTRealmtsigninitAllJtu)
if(!exists('sum.init.TchangeTave')) sum.init.TchangeTave <- summary(modrawTsdTTRealmtsigninitAllJtu)

sum.init.null
sum.init.Tchange
sum.init.TchangeTave
```
...
### Jtu models
```{r}
anova(modInitAllJtu, modRealmInitAllJtu)
anova(modInitAllJtu, modTaxamod2InitAllJtu)
anova(modInitAllJtu, modsdTtsignInitAllJtu)

anova(modInitAllJtu, modsdTRealmtsigninitAllJtu)
anova(modRealmInitAllJtu, modsdTRealmtsigninitAllJtu)
anova(modsdTtsignInitAllJtu, modsdTRealmtsigninitAllJtu)
...
### Horn models
```{r}
anova(modInitAllHorn, modRealmInitAllHorn)
anova(modInitAllHorn, modTaxamod2InitAllHorn)
anova(modInitAllHorn, modsdTtsignInitAllHorn)

anova(modInitAllHorn, modsdTRealmtsigninitAllHorn)
anova(modRealmInitAllHorn, modsdTRealmtsigninitAllHorn)
anova(modsdTtsignInitAllHorn, modsdTRealmtsigninitAllHorn)

anova(modsdTRealmtsigninitAllHorn, modrawTsdTTRealmtsigninitAllHorn)

```

### Covariate models
```{r}
anova(modrawTsdTTRealmtsigninitAllJtu, modrawTsdTTRealmtsignmicroclimInitAllJtu)
anova(modrawTsdTTRealmtsigninitAllJtu, modrawTsdTTRealmtsignhumanInitAllJtu)

```
## Plots
### 2D plots
made by pred_modrawXAllJtu.sh. Compare the plots below to models without init
```{r}
# read in slopes
...
slopesrawTsdTTRealmtsigninitJtu <- readRDS(here('temp', 'slopes_rawTsdTTRealmtsigninit.rds')) # made by pred_modrawXAllJtu.sh/.r
vals <- slopesrawTsdTTRealmtsigninitJtu[c(which.min(abs(tempave - 0)), which.min(abs(tempave - 10)), which.min(abs(tempave-25))), unique(tempave)] # show 0 and 25degC (+/-2SD from the mean)
slopesrawTsdTTRealmtsigninitJtu <- slopesrawTsdTTRealmtsigninitJtu[tempave %in% vals,]
slopesrawTsdTTRealmtsigninitJtu[, tempave := factor(as.character(round(tempave)), levels = c('0', '10', '25'))] # re-order factor for nicer plotting
