...
himp <- raster('dataDL/human_impact_marine/masked_model.tif') # marine human impact (Halpern)
...
bt[, cor.test(himp, atc, use = 'pairwise.complete.obs')]

# write out
write.csv(bt[, .(rarefyID, hfp, himp, atc)], file = gzfile('output/humanimpact_by_rarefyID.csv.gz'), row.names = FALSE)
