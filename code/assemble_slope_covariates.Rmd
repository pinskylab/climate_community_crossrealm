...
This is input to turnover_vs_temperature_MEmodels.Rmd
...
btinfo <- btinfo[!duplicated(rarefyID), .(rarefyID, STUDY_ID, rarefyID_x, rarefyID_y, REALM, Biome, taxa_mod)]

# biotime num individuals and other info
load(here::here('data', 'biotime_blowes', 'bt_malin.Rdata')) # load bt_malin
btnave <- data.table(bt_malin); rm(bt_malin)
btnave[, Nave := mean(N), by = rarefyID] # average number of individuals in the timeseries
btnave <- btnave[!duplicated(rarefyID), .(rarefyID, Nave)]
...
bt <- merge(bt, btnave, all.x = TRUE, by = 'rarefyID')
...
bt[, duration_group := factor(duration_group, levels = c('3annual', '4annual', '5annual', '6annual', '7annual', 
                                                         '8annual', '9annual', '10annual', '11annual', '12annual',
                                                         '13annual', '14annual', '15annual', '16annual', '17annual',
                                                         '18annual', '19annual', '20annual', 
                                                         '3min3', '4min3', '5min3', '6min3', '7min3', '8min3', '9min3',
                                                         '10min3', '11min3', '12min3', '13min3', '14min3', '15min3', 
                                                         '16min3', '17min3', '18min3', '19min3', '20min3',
                                                         'All', 'Ally1'), ordered = FALSE)]
...
## Identify core sets of studies
Sets of studies that are all present in a few different durations (e.g., 3, 5, 10 year durations)
```{r core studies}
core3min3_5min3_10min3char <- Reduce(intersect, list(btlong[measure == 'Jtu' & duration_group=='3min3', rarefyID],
                                          btlong[measure == 'Jtu' & duration_group=='5min3', rarefyID],
                                          btlong[measure == 'Jtu' & duration_group=='10min3', rarefyID])) # syntax from https://www.r-bloggers.com/2012/06/intersect-for-multiple-vectors-in-r/
core3min3_5min3_10min3_20min3char <- Reduce(intersect, list(btlong[measure == 'Jtu' & duration_group=='3min3', rarefyID],
                                          btlong[measure == 'Jtu' & duration_group=='5min3', rarefyID],
                                          btlong[measure == 'Jtu' & duration_group=='10min3', rarefyID],
                                          btlong[measure == 'Jtu' & duration_group=='20min3', rarefyID]))
length(core3min3_5min3_10min3_20min3char)
length(core3min3_5min3_10min3char)

bt[, core3min3_5min3_10min3 := FALSE]
bt[rarefyID %in% core3min3_5min3_10min3char & duration_group %in% c('3min3', '5min3', '10min3'), core3min3_5min3_10min3 := TRUE]
bt[, core3min3_5min3_10min3_20min3 := FALSE]
bt[rarefyID %in% core3min3_5min3_10min3_20min3char, core3min3_5min3_10min3_20min3 := TRUE]
```
...
if(!file.exists(here('output', 'slope_w_covariates.csv.gz'))){
  write.csv(bt[,.(STUDY_ID, rarefyID, rarefyID_y, REALM, Biome, taxa_mod, taxa_mod2, year1, year2, 
                  duration_group, duration, 
                  core3min3_5min3_10min3, core3min3_5min3_10min3_20min3,
...
invisible(bt[measure =='Jbeta' & duration_group == '3annual', hist(disstrend, main = 'Jaccard total 3-year annual slope', breaks = 80)])
invisible(bt[measure =='Jtu' & duration_group == '3annual', hist(disstrend, main = 'Jaccard turnover 3-year annual slope', breaks = 80)])
invisible(bt[measure =='Horn' & duration_group == '3annual', hist(disstrend, main = 'Morisita-Horn 3-year annual slope', breaks = 80)])

# histograms for 5-year ts
invisible(bt[measure =='Jbeta' & duration_group == '5annual', hist(disstrend, main = 'Jaccard total 5-year annual slope', breaks = 80)])
invisible(bt[measure =='Jtu' & duration_group == '5annual', hist(disstrend, main = 'Jaccard turnover 5-year annual slope', breaks = 80)])
invisible(bt[measure =='Horn' & duration_group == '5annual', hist(disstrend, main = 'Morisita-Horn 5-year annual slope', breaks = 80)])

# histograms for 10-year ts
invisible(bt[measure =='Jbeta' & duration_group == '10annual', hist(disstrend, main = 'Jaccard total 10-year annual slope', breaks = 80)])
invisible(bt[measure =='Jtu' & duration_group == '10annual', hist(disstrend, main = 'Jaccard turnover 10-year annual slope', breaks = 80)])
invisible(bt[measure =='Horn' & duration_group == '10annual', hist(disstrend, main = 'Morisita-Horn 10-year annual slope', breaks = 80)])

# histograms for 20-year ts
invisible(bt[measure =='Jbeta' & duration_group == '20annual', hist(disstrend, main = 'Jaccard total 20-year annual slope', breaks = 80)])
invisible(bt[measure =='Jtu' & duration_group == '20annual', hist(disstrend, main = 'Jaccard turnover 20-year annual slope', breaks = 80)])
invisible(bt[measure =='Horn' & duration_group == '20annual', hist(disstrend, main = 'Morisita-Horn 20-year annual slope', breaks = 80)])

# histograms for 3-year min3 ts
invisible(bt[measure =='Jbeta' & duration_group == '3min3', hist(disstrend, main = 'Jaccard total 3-year min3 slope', breaks = 80)])
invisible(bt[measure =='Jtu' & duration_group == '3min3', hist(disstrend, main = 'Jaccard turnover 3-year min3 slope', breaks = 80)])
invisible(bt[measure =='Horn' & duration_group == '3min3', hist(disstrend, main = 'Morisita-Horn 3-year min3 slope', breaks = 80)])

# histograms for 5-year min3 ts
invisible(bt[measure =='Jbeta' & duration_group == '5min3', hist(disstrend, main = 'Jaccard total 5-year min3 slope', breaks = 80)])
invisible(bt[measure =='Jtu' & duration_group == '5min3', hist(disstrend, main = 'Jaccard turnover 5-year min3 slope', breaks = 80)])
invisible(bt[measure =='Horn' & duration_group == '5min3', hist(disstrend, main = 'Morisita-Horn 5-year min3 slope', breaks = 80)])

# histograms for 10-year min3 ts
invisible(bt[measure =='Jbeta' & duration_group == '10min3', hist(disstrend, main = 'Jaccard total 10-year min3 slope', breaks = 80)])
invisible(bt[measure =='Jtu' & duration_group == '10min3', hist(disstrend, main = 'Jaccard turnover 10-year min3 slope', breaks = 80)])
invisible(bt[measure =='Horn' & duration_group == '10min3', hist(disstrend, main = 'Morisita-Horn 10-year min3 slope', breaks = 80)])

# histograms for 20-year min3 ts
invisible(bt[measure =='Jbeta' & duration_group == '20min3', hist(disstrend, main = 'Jaccard total 20-year min3 slope', breaks = 80)])
invisible(bt[measure =='Jtu' & duration_group == '20min3', hist(disstrend, main = 'Jaccard turnover 20-year min3 slope', breaks = 80)])
invisible(bt[measure =='Horn' & duration_group == '20min3', hist(disstrend, main = 'Morisita-Horn 20-year min3 slope', breaks = 80)])

# histograms for all ts
invisible(bt[measure =='Jbeta' & duration_group == 'All', hist(disstrend, main = 'Jaccard total all-year slope', breaks = 80)])
invisible(bt[measure =='Jtu' & duration_group == 'All', hist(disstrend, main = 'Jaccard turnover all-year slope', breaks = 80)])
invisible(bt[measure =='Horn' & duration_group == 'All', hist(disstrend, main = 'Morisita-Horn all-year slope', breaks = 80)])
...
### Initial year
```{r, echo=FALSE, warning = FALSE, message = FALSE, fig.width=14,fig.height=12}
ggplot(bt[measure == 'Jtu'], aes(year1, disstrend)) +
  geom_point(alpha = 0.1, aes(color = year2 - year1)) + 
  geom_smooth() +
  labs(x = 'Initial year', y = 'Jaccard turnover slope') +
  facet_wrap(vars(duration_group), scales = 'free')
```
...
### Initial year
```{r, echo=FALSE, warning = FALSE, message = FALSE, fig.width=14,fig.height=12}
ggplot(bt[measure == 'Jbeta'], aes(year1, disstrend)) +
  geom_point(alpha = 0.1, aes(color = year2 - year1)) + 
  geom_smooth() +
  labs(x = 'Initial year', y = 'Jaccard total slope') +
  facet_wrap(vars(duration_group), scales = 'free')
```
...
### Initial year
```{r, echo=FALSE, warning = FALSE, message = FALSE, fig.width=14,fig.height=12}
ggplot(bt[measure == 'Horn'], aes(year1, disstrend)) +
  geom_point(alpha = 0.1, aes(color = year2 - year1)) + 
  geom_smooth() +
  labs(x = 'Initial year', y = 'Horn slope') +
  facet_wrap(vars(duration_group), scales = 'free')
```
