...
print(paste0('trendsall has ', nrow(trendsall), ' rows'))


# missing some gains and losses
print(paste0('Number of gains-losses that are missing: ', trendsall[, sum(is.na(gainlossprop))]))
...
iallJtu <- downsampRarefyID(dat = trendsall, inds = iallJtu, seed = bootID)
...
iallHorn <- downsampRarefyID(dat = trendsall, inds = iallHorn, seed = bootID)
...
iGLallJtu <- downsampRarefyID(dat = trendsall, inds = iGLallJtu, seed = bootID)
...
iallJtuterr <- downsampRarefyID(dat = trendsall, inds = iallJtuterr, seed = bootID)
...
if (MATCHMOD) {
    print(summary(mod))
    outfile <- paste0('temp/', fitmod, '_boot', bootID, '.rds')
