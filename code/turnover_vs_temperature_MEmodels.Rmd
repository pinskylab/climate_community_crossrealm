...
---
# Prep
Uses output from assemble_slope_covariates.Rmd
...
trends[, duration_factor := as.factor(year2 - year1 + 1)]
...
# Models
These models have a high false positive rate if fit to datasets with timeseries of different durations (see duration_sim.Rmd).
...
## Antao-style models of temptrend by standardized duration
Fit to core dataset of the same rarefyIDs across durations (3,5,10) or to all durations and all datasets. Use an interaction with duration so that coefficients can differ.
```{r}
if(file.exists(here('temp', 'modAntaoJtu3_5_10min3.rds'))){
  modAntaoJtu3_5_10min3 <- readRDS(here('temp','modAntaoJtu3_5_10min3.rds'))
} else {
  modAntaoJtu3_5_10min3 <- glmmTMB(disstrend ~ 0 + duration_factor:REALM + 
                                     duration_factor:tempave_metab.sc:REALM + 
                                     duration_factor:temptrend.sc:REALM + 
                                     duration_factor:temptrend.sc:tempave_metab.sc:REALM +
                                     (1|duration_factor/taxa_mod2/STUDY_ID),
                                   disp = ~trendse*REALM + duration_factor*REALM,
                                   data = trends[measure == 'Jtu' & core3min3_5min3_10min3 == TRUE,],
                                   control = glmmTMBControl(optCtrl=list(iter.max=1e3,eval.max=1e3)))
  saveRDS(modAntaoJtu3_5_10min3, file = here('temp','modAntaoJtu3_5_10min3.rds'))
}

if(file.exists(here('temp', 'modAntaoJtu3_5_10_20min3.rds'))){
  modAntaoJtu3_5_10_20min3 <- readRDS(here('temp','modAntaoJtu3_5_10_20min3.rds'))
} else {
  modAntaoJtu3_5_10_20min3 <- glmmTMB(disstrend ~ 0 + duration_factor:REALM + 
                                        duration_factor:tempave_metab.sc:REALM + 
                                        duration_factor:temptrend.sc:REALM + 
                                        duration_factor:temptrend.sc:tempave_metab.sc:REALM +
                                        (1|duration_factor/taxa_mod2/STUDY_ID),
                                      disp = ~trendse*REALM + duration_factor*REALM,
                                      data = trends[measure == 'Jtu' & core3min3_5min3_10min3_20min3 == TRUE,],
                                      control = glmmTMBControl(optCtrl=list(iter.max=1e4,eval.max=1e4)))
  saveRDS(modAntaoJtu3_5_10_20min3, file = here('temp','modAntaoJtu3_5_10_20min3.rds'))
}

if(file.exists(here('temp', 'modAntaoJtu3_5_10min3All.rds'))){
  modAntaoJtu3_5_10min3All <- readRDS(here('temp','modAntaoJtu3_5_10min3All.rds'))
} else {
  modAntaoJtu3_5_10min3All <- glmmTMB(disstrend ~ 0 + duration_factor:REALM + # all datasets, not sure the core
                                        duration_factor:tempave_metab.sc:REALM +
                                        duration_factor:temptrend.sc:REALM +
                                        duration_factor:temptrend.sc:tempave_metab.sc:REALM +
                                        (1|duration_factor/taxa_mod2/STUDY_ID),
                                      disp = ~trendse*REALM + duration_factor*REALM,
                                      data = trends[measure == 'Jtu' & duration_group %in% paste0(c(3,5,10), 'min3'),],
                                      control = glmmTMBControl(optCtrl=list(iter.max=1e3,eval.max=1e3)))
  saveRDS(modAntaoJtu3_5_10min3All, file = here('temp','modAntaoJtu3_5_10min3All.rds'))
}

if(file.exists(here('temp', 'modAntaoJtu3_5_10_20min3All.rds'))){
  modAntaoJtu3_5_10_20min3All <- readRDS(here('temp','modAntaoJtu3_5_10_20min3All.rds'))
} else {
  modAntaoJtu3_5_10_20min3All <- glmmTMB(disstrend ~ 0 + duration_factor:REALM + # all datasets, not sure the core
                                           duration_factor:tempave_metab.sc:REALM +
                                           duration_factor:temptrend.sc:REALM +
                                           duration_factor:temptrend.sc:tempave_metab.sc:REALM +
                                           (1|duration_factor/taxa_mod2/STUDY_ID),
                                         disp = ~trendse*REALM + duration_factor*REALM,
                                         data = trends[measure == 'Jtu' & duration_group %in% paste0(c(3,5,10,20), 'min3'),],
                                         control = glmmTMBControl(optCtrl=list(iter.max=1e3,eval.max=1e3)))
  saveRDS(modAntaoJtu3_5_10_20min3All, file = here('temp','modAntaoJtu3_5_10_20min3All.rds'))
}

summary(modAntaoJtu3_5_10min3)
#summary(modAntaoJtuXmin3)

```

### Plot coefficients
```{r, echo = FALSE}
coefs <- fixef(modAntaoJtu3_5_10min3)$cond
iM <- grep('REALMMarine:tempave_metab.sc:temptrend.sc', names(coefs))
dursM <- as.numeric(gsub('duration_factor|:REALMMarine:tempave_metab.sc:temptrend.sc', '', names(coefs)[iM]))
iT <- grep('REALMTerrestrial:tempave_metab.sc:temptrend.sc', names(coefs))
dursT <- as.numeric(gsub('duration_factor|:REALMTerrestrial:tempave_metab.sc:temptrend.sc', '', names(coefs)[iT]))
par(mfrow=c(1,2))
plot(dursM, coefs[iM], type = 'l', xlab = 'Time series duration', ylab = 'AveTemp:TempTrend interaction', main = "Marine core3-5-10min3")
abline(h =0, lty = 2)
plot(dursT, coefs[iT], type = 'l', xlab = 'Time series duration', ylab = 'AveTemp:TempTrend interaction', main = 'Terrestrial core3-5-10min3')
abline(h =0, lty = 2)

# coefs <- fixef(modAntaoJtu3_5_10_20min3)$cond
# iM <- grep('REALMMarine:tempave_metab.sc:temptrend.sc', names(coefs))
# dursM <- as.numeric(gsub('duration_factor|:REALMMarine:tempave_metab.sc:temptrend.sc', '', names(coefs)[iM]))
# iT <- grep('REALMTerrestrial:tempave_metab.sc:temptrend.sc', names(coefs))
# dursT <- as.numeric(gsub('duration_factor|:REALMTerrestrial:tempave_metab.sc:temptrend.sc', '', names(coefs)[iT]))
# par(mfrow=c(1,2))
# plot(dursM, coefs[iM], type = 'l', xlab = 'Time series duration', ylab = 'AveTemp:TempTrend interaction', main = "Marine core3-5-10-20min3")
# abline(h =0, lty = 2)
# plot(dursT, coefs[iT], type = 'l', xlab = 'Time series duration', ylab = 'AveTemp:TempTrend interaction', main = 'Terrestrial core3-5-10-20min3')
# abline(h =0, lty = 2)

coefs <- fixef(modAntaoJtu3_5_10min3All)$cond
iM <- grep('REALMMarine:tempave_metab.sc:temptrend.sc', names(coefs))
dursM <- as.numeric(gsub('duration_factor|:REALMMarine:tempave_metab.sc:temptrend.sc', '', names(coefs)[iM]))
iT <- grep('REALMTerrestrial:tempave_metab.sc:temptrend.sc', names(coefs))
dursT <- as.numeric(gsub('duration_factor|:REALMTerrestrial:tempave_metab.sc:temptrend.sc', '', names(coefs)[iT]))
par(mfrow=c(1,2))
plot(dursM, coefs[iM], type = 'l', xlab = 'Time series duration', ylab = 'AveTemp:TempTrend interaction', main = "Marine all 3-5-10min3")
abline(h =0, lty = 2)
plot(dursT, coefs[iT], type = 'l', xlab = 'Time series duration', ylab = 'AveTemp:TempTrend interaction', main = 'Terrestrial all min3')
abline(h =0, lty = 2)

# coefs <- fixef(modAntaoJtu3_5_10_20min3All)$cond
# iM <- grep('REALMMarine:tempave_metab.sc:temptrend_abs.sc', names(coefs))
# dursM <- as.numeric(gsub('duration_factor|:REALMMarine:tempave_metab.sc:temptrend.sc', '', names(coefs)[iM]))
# iT <- grep('REALMTerrestrial:tempave_metab.sc:temptrend.sc', names(coefs))
# dursT <- as.numeric(gsub('duration_factor|:REALMTerrestrial:tempave_metab.sc:temptrend.sc', '', names(coefs)[iT]))
# par(mfrow=c(1,2))
# plot(dursM, coefs[iM], type = 'l', xlab = 'Time series duration', ylab = 'AveTemp:TempTrend interaction', main = "Marine all 3-5-10-20min3")
# abline(h =0, lty = 2)
# plot(dursT, coefs[iT], type = 'l', xlab = 'Time series duration', ylab = 'AveTemp:TempTrend interaction', main = 'Terrestrial all min3')
# abline(h =0, lty = 2)
...
## Antao-style models of abs(temptrend) by standardized duration
Fit to core dataset of the same rarefyIDs across durations (3,5,10) or to all durations and all datasets. Use an interaction with duration so that 
```{r}
if(file.exists(here('temp', 'modAntaoAbsJtu3_5_10min3.rds'))){
  modAntaoAbsJtu3_5_10min3 <- readRDS(here('temp','modAntaoAbsJtu3_5_10min3.rds'))
} else {
  modAntaoAbsJtu3_5_10min3 <- glmmTMB(disstrend ~ 0 + duration_factor:REALM + 
                                        duration_factor:tempave_metab.sc:REALM + 
                                        duration_factor:temptrend_abs.sc:REALM + 
                                        duration_factor:temptrend_abs.sc:tempave_metab.sc:REALM +
                                        (1|duration_factor/taxa_mod2/STUDY_ID),
                                      disp = ~trendse*REALM + duration_factor*REALM,
                                      data = trends[measure == 'Jtu' & core3min3_5min3_10min3 == TRUE,],
                                      control = glmmTMBControl(optCtrl=list(iter.max=1e3,eval.max=1e3)))
  saveRDS(modAntaoAbsJtu3_5_10min3, file = here('temp','modAntaoAbsJtu3_5_10min3.rds'))
}

# if(file.exists(here('temp', 'modAntaoAbsJtu3_5_10_20min3.rds'))){
#   modAntaoAbsJtu3_5_10_20min3 <- readRDS(here('temp','modAntaoAbsJtu3_5_10_20min3.rds'))
# } else {
#   modAntaoAbsJtu3_5_10_20min3 <- glmmTMB(disstrend ~ 0 + duration_factor:REALM + 
#                                         duration_factor:tempave_metab.sc:REALM + 
#                                         duration_factor:temptrend_abs.sc:REALM + 
#                                         duration_factor:temptrend_abs.sc:tempave_metab.sc:REALM +
#                                         (1|duration_factor/taxa_mod2/STUDY_ID),
#                                       disp = ~trendse*REALM + duration_factor*REALM,
#                                       data = trends[measure == 'Jtu' & core3min3_5min3_10_20min3 == TRUE,],
#                                       control = glmmTMBControl(optCtrl=list(iter.max=1e3,eval.max=1e3)))
#   saveRDS(modAntaoAbsJtu3_5_10_20min3, file = here('temp','modAntaoAbsJtu3_5_10_20min3.rds'))
# }

if(file.exists(here('temp', 'modAntaoAbsJtu3_5_10min3All.rds'))){
  modAntaoAbsJtu3_5_10min3All <- readRDS(here('temp','modAntaoAbsJtu3_5_10min3All.rds'))
} else {
  modAntaoAbsJtu3_5_10min3All <- glmmTMB(disstrend ~ 0 + duration_factor:REALM + # all datasets, not sure the core
                                        duration_factor:tempave_metab.sc:REALM +
                                        duration_factor:temptrend_abs.sc:REALM +
                                        duration_factor:temptrend_abs.sc:tempave_metab.sc:REALM +
                                        (1|duration_factor/taxa_mod2/STUDY_ID),
                                      ...
                                      data = trends[measure == 'Jtu' & duration_group %in% paste0(c(3,5,10), 'min3'),],
                                      control = glmmTMBControl(optCtrl=list(iter.max=1e3,eval.max=1e3)))
  saveRDS(modAntaoAbsJtu3_5_10min3All, file = here('temp','modAntaoAbsJtu3_5_10min3All.rds'))
}

# modAntaoAbsJtuXmin3 <- glmmTMB(disstrend ~ 0 + REALM:duration_factor + 
#                              tempave_metab.sc:REALM:duration_factor + 
#                              temptrend_abs.sc:REALM:duration_factor + 
#                              temptrend_abs.sc:tempave_metab.sc:REALM:duration_factor +
#                          (1|duration_factor/taxa_mod2/STUDY_ID),
#                          disp = ~trendse*REALM + REALM*duration_factor,
#                          data = trends[measure == 'Jtu' & duration_group %in% paste0(3:20, 'min3'),],
#                          control = glmmTMBControl(optCtrl=list(iter.max=1e3,eval.max=1e3)))

summary(modAntaoAbsJtu3_5_10min3)
# summary(modAntaoAbsJtuXmin3)

```

### Compare
```{r}
AIC(modAntaoAbsJtu3_5_10min3, modAntaoJtu3_5_10min3) - min(AIC(modAntaoAbsJtu3_5_10min3, modAntaoJtu3_5_10min3))
AIC(modAntaoAbsJtu3_5_10min3All, modAntaoJtu3_5_10min3All) - min(AIC(modAntaoAbsJtu3_5_10min3All, modAntaoJtu3_5_10min3All))
```

### Plot coefficients
```{r, echo = FALSE}
coefs <- fixef(modAntaoAbsJtu3_5_10min3)$cond
iM <- grep('REALMMarine:tempave_metab.sc:temptrend_abs.sc', names(coefs))
dursM <- as.numeric(gsub('duration_factor|:REALMMarine:tempave_metab.sc:temptrend_abs.sc', '', names(coefs)[iM]))
iT <- grep('REALMTerrestrial:tempave_metab.sc:temptrend_abs.sc', names(coefs))
dursT <- as.numeric(gsub('duration_factor|:REALMTerrestrial:tempave_metab.sc:temptrend_abs.sc', '', names(coefs)[iT]))
par(mfrow=c(1,2))
plot(dursM, coefs[iM], type = 'l', xlab = 'Time series duration', ylab = 'AveTemp:TempTrend interaction', main = "Marine core3-10min3")
abline(h =0, lty = 2)
plot(dursT, coefs[iT], type = 'l', xlab = 'Time series duration', ylab = 'AveTemp:TempTrend interaction', main = 'Terrestrial core3-10min3')
abline(h =0, lty = 2)

coefs <- fixef(modAntaoAbsJtu3_5_10min3All)$cond
iM <- grep('REALMMarine:tempave_metab.sc:temptrend_abs.sc', names(coefs))
dursM <- as.numeric(gsub('duration_factor|:REALMMarine:tempave_metab.sc:temptrend_abs.sc', '', names(coefs)[iM]))
iT <- grep('REALMTerrestrial:tempave_metab.sc:temptrend_abs.sc', names(coefs))
dursT <- as.numeric(gsub('duration_factor|:REALMTerrestrial:tempave_metab.sc:temptrend_abs.sc', '', names(coefs)[iT]))
par(mfrow=c(1,2))
plot(dursM, coefs[iM], type = 'l', xlab = 'Time series duration', ylab = 'AveTemp:TempTrend interaction', main = "Marine all 3-10min3")
abline(h =0, lty = 2)
plot(dursT, coefs[iT], type = 'l', xlab = 'Time series duration', ylab = 'AveTemp:TempTrend interaction', main = 'Terrestrial all 3-10min3')
abline(h =0, lty = 2)

# coefs <- fixef(modAntaoJtuXmin3)$cond
# iM <- grep('REALMMarine:tempave_metab.sc:temptrend_abs.sc', names(coefs))
# dursM <- as.numeric(gsub('duration_factor|:REALMMarine:tempave_metab.sc:temptrend_abs.sc', '', names(coefs)[iM]))
# iT <- grep('REALMTerrestrial:tempave_metab.sc:temptrend_abs.sc', names(coefs))
# dursT <- as.numeric(gsub('duration_factor|:REALMTerrestrial:tempave_metab.sc:temptrend_abs.sc', '', names(coefs)[iT]))
# par(mfrow=c(1,2))
# plot(dursM, coefs[iM], type = 'l', xlab = 'Time series duration', ylab = 'AveTemp:TempTrend interaction', main = "Marine all min3")
# abline(h =0, lty = 2)
# plot(dursT, coefs[iT], type = 'l', xlab = 'Time series duration', ylab = 'AveTemp:TempTrend interaction', main = 'Terrestrial all min3')
# abline(h =0, lty = 2)
