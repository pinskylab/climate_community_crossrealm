...
mods[[13]] <- lme(fixed, data = trends[i,], random = list(taxa_mod2 = ~ tempchange_abs.sc, STUDY_ID = ~ tempchange_abs.sc, 
                                                          rarefyID = ~tempchange_abs.sc, compID = ~1))
mods[[14]] <- lme(fixed, data = trends[i,], random = list(STUDY_ID = ~ tempchange_abs.sc, rarefyID = ~tempchange_abs.sc))
mods[[15]] <- lme(fixed, data = trends[i,], random = list(STUDY_ID = ~ tempchange_abs.sc, rarefyID = ~tempchange_abs.sc, compID = ~1)) # includes overdispersion. new formula so that random slope is only for study level (not enough data to extend to compID).

mods[[16]] <- lme(fixed, data = trends[i,], random = list(STUDY_ID = ~tempchange_abs.sc, rarefyID = ~tempchange_abs.sc), 
                  weights = varPower(-0.5, ~tempchange_abs.sc))
mods[[17]] <- lme(fixed, data = trends[i,], random = list(taxa_mod2 = ~ tempchange_abs.sc, STUDY_ID = ~ tempchange_abs.sc, rarefyID = ~tempchange_abs.sc), 
                  weights = varPower(-0.5, ~tempchange_abs.sc))
mods[[18]] <- lme(fixed, data = trends[i,], random = list(taxa_mod2 = ~ tempchange_abs.sc, STUDY_ID = ~ tempchange_abs.sc, 
                                                          rarefyID = ~tempchange_abs.sc, compID = ~1), 
                  weights = varPower(-0.5, ~tempchange_abs.sc))
mods[[19]] <- lme(fixed, data = trends[i,], random = list(STUDY_ID = ~ tempchange_abs.sc, 
                                                          rarefyID = ~tempchange_abs.sc, compID = ~1), 
                  weights = varPower(-0.5, ~tempchange_abs.sc))
...
which.min(aics)
```
Chooses the random slopes (tempchange_abs) & intercepts for STUDY_ID, overdispersion, and variance scaled to number of years.
...
randef <- list(STUDY_ID = ~ abs(tempchange), rarefyID = ~1)
...
if(file.exists('temp/modonlyTtrendrem0.rds')){
  ...
  i <- trends[, complete.cases(Jtutrendrem0, REALM, tempchange)]
  modonlyTtrendrem0 <- lme(Jtutrendrem0 ~ abs(tempchange)*REALM,
...
if(file.exists('temp/modonlyTtrendJbetarem0.rds')){
  ...
  i <- trends[, complete.cases(Jbetatrendrem0, REALM, tempchange)]
  modonlyTtrendJbetarem0 <- lme(Jbetatrendrem0 ~ abs(tempchange)*REALM,
                   random = randef, weights = varef, data = trends[i,], method = 'REML', 
...
if(file.exists('temp/modonlyTtrendHornrem0.rds')){
  ...
  i <- trends[, complete.cases(Horntrendrem0, REALM, tempchange)]
  modonlyTtrendHornrem0 <- lme(Horntrendrem0 ~ abs(tempchange)*REALM,
                   random = randef, weights = varef, data = trends[i,], method = 'REML')
...
if(file.exists('temp/modTDtrendJturem0.rds')){
  ...
  i <- trends[, complete.cases(Jtutrendrem0, REALM, tempchange)]
  modTDtrendJturem0 <- lme(Jtutrendrem0 ~ abs(tempchange)*REALM + duration,
                   random = randef, weights = varef, data = trends[i,], method = 'REML')
...
if(file.exists('temp/modTDtrendJbetarem0.rds')){
  ...
  i <- trends[, complete.cases(Jbetatrendrem0, REALM, tempchange)]
  modTDtrendJbetarem0 <- lme(Jbetatrendrem0 ~ abs(tempchange)*REALM + duration,
                   random = randef, weights = varef, data = trends[i,], method = 'REML', 
                   control=lmeControl(msMaxIter = 100, maxIter = 100))
...
if(file.exists('temp/modTDtrendHornrem0.rds')){
  ...
  i <- trends[, complete.cases(Horntrendrem0, REALM, tempchange)]
  modTDtrendHornrem0 <- lme(Horntrendrem0 ~ abs(tempchange)*REALM + duration,
                   random = randef, weights = varef, data = trends[i,], method = 'REML')
