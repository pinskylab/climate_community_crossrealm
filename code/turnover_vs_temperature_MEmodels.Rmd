...
## Antao-style models
```{r}
randef <- '+(0 + temptrend.sc|taxa_mod) +(1|taxa_mod/STUDY_ID)'
#randef <- '+(1|taxa_mod/STUDY_ID)'
disp <- '~trendse'
cont <- glmmTMBControl(optCtrl=list(iter.max=1e3,eval.max=1e3))

fixed0 <- 'disstrend ~ 0 + tempave_metab.sc:REALM + temptrend.sc:REALM + temptrend.sc:tempave_metab.sc:REALM' # like Antao. No turnover at no temptrend.
fixed1 <- 'disstrend ~ 0 + REALM + tempave_metab.sc:REALM + temptrend.sc:REALM + temptrend.sc:tempave_metab.sc:REALM' # allow turnover at no temptrend.
trendsAllJtu <- trends[complete.cases(disstrend, REALM, tempave_metab.sc, temptrend.sc) & 
                       measure == 'Jtu' & duration_group == 'All', ]
trends5Jtu <- trends[complete.cases(disstrend, REALM, tempave_metab.sc, temptrend.sc) & 
