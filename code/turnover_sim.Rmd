...
set.seed(5)
data('BCI')
metaSAD = rep(1:ncol(BCI), colSums(BCI)) # metacommunity
...
# simulation
for(i in 1:nrep){
    simburninSAD[[i]] <- untb(start = as.count(sample(metaSAD, nindiv)), prob = prob, D = 1, gens = nburn, keep = TRUE, meta = as.count(metaSAD)) # burnin
    simSAD[[i]] <- untb(start = simburninSAD[[i]][nburn,], prob = prob, D=1, gens = ngen, keep = TRUE, meta = as.count(metaSAD)) # simulation
}


# plots
plot(as.count(metaSAD), main = 'Regional SAD')
...
plot(count(simburninSAD[[1]][1,]), main = 'Burnin: Initial local SAD')
plot(count(simburninSAD[[1]][nburn,]), main = 'Burnin: Final local SAD')
...
plot(count(simSAD[[1]][ngen,]), main = 'Sim: Final local SAD')
