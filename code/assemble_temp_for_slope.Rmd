...
```{r}
tslopes[REALM == 'Terrestrial', .(na = sum(is.na(trend_cruts)), n = sum(!is.na(trend_cruts))), by = .(rarefyID)][na > 0,]
tslopes[REALM == 'Freshwater', .(na = sum(is.na(trend_cruts)), n = sum(!is.na(trend_cruts))), by = .(rarefyID)][na > 0,]
tslopes[REALM == 'Marine', .(na = sum(is.na(trend_cruts)), n = sum(!is.na(trend_cruts))), by = .(rarefyID)][na > 0, .N]


ggplot(tslopes[REALM %in% c('Terrestrial', 'Freshwater'), ], aes(lon_cruts, lat_cruts, color = is.na(trend_cruts))) +
    geom_point(size = 0.2)
...
```{r}
tslopes[REALM == 'Terrestrial', .(na = sum(is.na(trend_ersst)), n = sum(!is.na(trend_ersst))), by = .(rarefyID)][na > 0,]
tslopes[REALM == 'Freshwater', .(na = sum(is.na(trend_ersst)), n = sum(!is.na(trend_ersst))), by = .(rarefyID)][na > 0,]
tslopes[REALM == 'Marine', .(na = sum(is.na(trend_ersst)), n = sum(!is.na(trend_ersst))), by = .(rarefyID)][na > 0, .N]


ggplot(tslopes[REALM %in% c('Marine'), ], aes(lon_ersst, lat_ersst, color = is.na(trend_ersst))) +
    geom_point(size = 0.2)
```
