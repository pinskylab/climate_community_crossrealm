...
# Script to fit glmmTMB modrawTsdTTRealmtsignAllJbeta models
# nohup code/turnover_vs_temperature_GLMM_fit_modrawTsdTTRealmtsignAllJbeta.R XX > logs/turnover_vs_temperature_GLMMXX.Rout &
# where XX is a model name (see options below)
# (this works if code is executable, e.g., chmod u+x code/turnover_vs_temperature_GLMM_fit_modrawTsdTTRealmtsignAllJbeta.R)
...
print(paste('This is script turnover_vs_temperature_GLMM_fit_modrawTsdTTRealmtsignAllJbeta.R'))
...
if (fitmod == 'modsdTRealmtsignAllJbeta') {
    if (MATCHMOD)
...
if (fitmod == 'modrawTsdTTRealmtsignAllJbeta') {
    if (MATCHMOD)
        stop('Model name matched more than one model!')
    print(paste(sum(iallJbeta), 'data points'))
    mod <- glmmTMB(
