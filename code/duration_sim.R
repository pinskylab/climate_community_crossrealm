...
cors <- data.table(n = 1000, minduration = rep(c(10,3,3,3,3), nreps), maxduration = rep(c(10, 12, 27, 52, 102), nreps), cor.p = NA_real_, cor.cor = NA_real_, lm.m = NA_real_,
                   ...
                   glmmonebeta.p = NA_real_, glmmonebeta.beta = NA_real_) # holds the summaries from each dataset. use n = 1000 timeseries per dataset.
#cors2 <- copy(cors)
...
for(i in 1:nrow(cors)){
    ...
    dat[, ':='(obsid = 1:.N, tslope = runif(1, 0.005, 0.1)), by = tsid] # time counter and a random slope for each tsid
    dat[, ':='(value1.logit = obsid*tslope + rnorm(.N), value2 = rnorm(.N))] # create random ts. value1 has a positive slope like dissimilarities and is on a logit scale.
