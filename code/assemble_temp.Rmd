...
```{r}
# CRU TS
btseastmp <- merge(bttmp[!duplicated(rarefyID), .(rarefyID, lat_cruts, lon_cruts)], tmpseas, by = c('lat_cruts', 'lon_cruts')) # merge bt with seasonality values
write.csv(btseastmp, file = gzfile('output/cruts_seas.csv.gz')) # write out

# ERSST
btseassst <- merge(btsst[!duplicated(rarefyID), .(rarefyID, lat_ersst, lon_ersst)], sstseas, by = c('lat_ersst', 'lon_ersst'))
