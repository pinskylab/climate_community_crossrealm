...
```{r}
# CRU TS
btallyrs <- bt[!is.na(tmp), .(YEAR = seq(min(YEAR), max(YEAR), by = 1)), by = .(rarefyID, lat, lon)] # make a data.table of all years from first to last for each BioTime dataset with tmp data
btallyrs <- merge(btallyrs, tmpyr, by = c('lat', 'lon', 'YEAR'), all.x = TRUE) # merge the data.table with tmp values
out3 <- btallyrs[, .(tmptrendallyr = coef(lm(tmp ~ YEAR))[2], YEAR_min = min(YEAR), YEAR_max = max(YEAR)), 
          by = .(rarefyID)] # calculate trends
write.csv(out3, file = gzfile('output/cruts_tmptrend_allyear.csv.gz')) # write out

# ERSST
btallyrssst <- btsst[!is.na(sst), .(YEAR = seq(min(YEAR), max(YEAR), by = 1)), by = .(rarefyID, lat, lon)]
btallyrssst <- merge(btallyrssst, sstyr, by = c('lat', 'lon', 'YEAR'), all.x = TRUE)
...
```{r}
# CRU TS
btseas <- merge(bt[!duplicated(rarefyID), .(rarefyID, lat, lon)], tmpseas, by = c('lat', 'lon'), all.x = TRUE) # merge bt with seasonality values
write.csv(btseas, file = gzfile('output/cruts_seas.csv.gz')) # write out

# ERSST
btseassst <- merge(btsst[!duplicated(rarefyID), .(rarefyID, lat, lon)], sstseas, by = c('lat', 'lon'), all.x = TRUE)
