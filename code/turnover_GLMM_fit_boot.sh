...
# As arguments, provide (first) the bootstrap ID and then the names of models to fit, e.g.,
# turnover_GLMM_fit_boot.sh 1 modAllJtu modAllJbeta

echo "You asked to run $(($#-1)) model(s) with bootID $1"
bootID=$1
...
while [ "$1" != "" ]; do
    echo "Spawning $1"
    nohup code/turnover_GLMM_fit_boot.R $1 $bootID > logs/turnover_GLMM$1_boot$bootID.Rout &
