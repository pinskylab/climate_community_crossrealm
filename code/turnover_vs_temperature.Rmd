...
```{r}
trends <- bt[, .(Jtutrend = coef(lm(Jtu_base ~ YEAR))[2], Strend = coef(lm(S ~ YEAR))[2], nyrBT = length(YEAR)), by = .(REALM, Biome, taxa_mod, STUDY_ID, rarefyID)] # calculate trend in Jaccard turnover from first year
...
ggplot(trends, aes(tmptrendallyr, ssttrendbyyr)) +
    geom_point() +
    geom_smooth()
...
### Plot Richness trend vs. temperature trends (for now, just have temperature data on land).
```{r}
...
### Model choice
Realm interaction is strongly supported
Baseline turnover is slowest in freshwater, fastest in marine
Freshwater turnover is fastest with temperature, terrestrial slowest
```{r}
# Jaccard turnover trend vs. temperature trend (across all years)
mod1 <- lm(Jtutrend ~ abs(temptrend_comb_allyr) + REALM, data = trends)
mod2 <- lm(Jtutrend ~ abs(temptrend_comb_allyr)*REALM, data = trends)
