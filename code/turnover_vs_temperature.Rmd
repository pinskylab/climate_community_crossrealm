...
### Plot turnover vs. temperature trends (for now, just have temperature data on land).
...
ggplot(trends, aes(tmptrendallyr, Strend, color = REALM)) +
    geom_point(size = 0.2, alpha = 0.5) + 
    geom_smooth()

# Richness trend vs. abs temperature trend (across all years)
ggplot(trends, aes(abs(tmptrendallyr), Strend, group = REALM, color = REALM)) +
    geom_point(size = 0.2, alpha = 0.5) + 
    geom_smooth()
```