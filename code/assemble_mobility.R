...
# read in data
mass <- fread('output/mass_byspecies.csv.gz')
phylo <- fread('output/taxonomy.csv.gz')

# merge in taxonomy
phylo[, original_name := gsub(' ', '_', original_name)]
phylo[, ':='(gateway.masses = NULL, V1 = NULL, X = NULL, NA. = NULL)]
...
mob[is.na(mode) & !duplicated(species), table(phylum)]
...
mob[, speed := NA_real_]
mob[mode == 'flying', speed := 142.8 * mass^0.24 * (1 - exp(-2.4 * mass^(-0.72)))]
mob[mode == 'running', speed := 25.5 * mass^0.26 * (1 - exp(-22 * mass^(-0.6)))]
mob[mode == 'crawling', speed := 0.1] # below the slowest runners
mob[mode == 'swimming', speed := 11.2 * mass^0.36 * (1 - exp(-19.5 * mass^(-0.56)))]
...
mob[, .(n = length(unique(original_name)), val = sum(!is.na(speed))), by = rarefyID][(val/n) < 0.5, ] # 1278 rarefyID with <50%
...
priorities <- mob[, .(n = length(unique(original_name)), val = sum(!is.na(speed))), by = 
