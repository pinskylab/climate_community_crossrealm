...
combXmin3 <- merge(dat[model_id == 'logS_lm', .(rarefyID, new_sTempYear, TempGAMCoef, taxa_mod1)], trends[grep('min3', duration_group), !c("rarefyID_y" ,"duration")], by = c('rarefyID'), all = TRUE)
combXmin3[, ':='(duration = year2 - year1 + 1, duration_factor = as.factor(year2 - year1 + 1))]
...
- All durations in one model, but allow slopes to differ by duration. Uses abs(temptrend)
...
# predict
newdatM$slope <- predict(modJbetaXmin3abs, newdata = newdatM, se.fit = FALSE, re.form = NA)
newdatT$slope <- predict(modJbetaXmin3abs, newdata = newdatT, se.fit = FALSE, re.form = NA)
